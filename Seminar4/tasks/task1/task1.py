...
for i in range(1, N):
	print(rank)
	prev_array = show_array[i-1].copy()
	cur_array = show_array[i].copy()
	prev_array = comm.bcast(prev_array, root=0)
	cur_array = comm.bcast(cur_array, root=0)


state = int(f'{int(prev_array[rank - 1 + N if rank - 1 < 0 else rank - 1])}{int(prev_array[rank])}{int(prev_array[rank + 1 - N if rank + 1 >= N  else rank + 1])}', 2)

match state:
	case 0:
		array[rank] = bool(int(s_rule[7]))
	case 1:
		array[rank] = bool(int(s_rule[6]))
	case 2:
		array[rank] = bool(int(s_rule[5]))
	case 3:
		array[rank] = bool(int(s_rule[4]))
	case 4:
		array[rank] = bool(int(s_rule[3]))
	case 5:
		array[rank] = bool(int(s_rule[2]))
	case 6:
		array[rank] = bool(int(s_rule[1]))
	case 7:
		array[rank] = bool(int(s_rule[0]))
